...
map.forEach((value, key) => {
    try {
        document.getElementById("demo").innerHTML += "<tr><td>" + `${key}` + "</td><td>" + `${value}` + "</td></tr>";
    } catch (e) {
...
map.forEach((value, key) => {
    try {
        document.getElementById("drugName").innerHTML += "<option value=\"" + `${key}` + "\">" +  `${key}` + "</option>";
    } catch (e) {
...
try {
    document.getElementById("random").innerHTML += Math.floor((Math.random() * 10000000000) + 1);
} catch (e) {
...
var validateInput = () => {
    try {
        if(document.getElementById("citizen_yes").checked && document.getElementById("insurance_yes").checked) {
             return true;
        }
        document.getElementById("errormsg").innerHTML = "I'm sorry you're not eligible for this program!";
    } catch (e) {
