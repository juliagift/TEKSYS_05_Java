package com.example.requestprama.controller;
...
public class PathVariableController {
	 ...
	 /*
	    ...
	    */
	@GetMapping(value="/request1/{name}")
	@ResponseBody
	...
	/*  If the method parameter type is not String then Spring automatically converts to 
	 * the appropriate type such as int, long, float, etc.   */
	@GetMapping("/request3/{name}/{age}/{salary}/{active}")
	@ResponseBody
	public String handler(@PathVariable("name") String name, @PathVariable("age") int age, @PathVariable("salary") double salary, @PathVariable("active") boolean active) {
		return "URL parameters - <br>"
				+ "name = " + name + " <br>"
				+ "age = " + age + " <br>"
				+ "salary = " + salary + " <br>"
